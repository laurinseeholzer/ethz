...
\begin{remark}
    ...
    \begin{itemize}
        ...
        \item $\lim_{n \to \infty} \frac{n^k}{a^n} = 0 \quad (a \gt 1)$
        \item $\lim_{n \to \infty} \frac{\log_a n}{n^k} = 0 \quad (a \gt 1, k \gt 0)$
...
\begin{theorem}
    ...
    \begin{itemize}
        \item \textbf{Bernoulli's Inequality:} $(1+x)^n \ge 1+nx$ (for $x \ge -1$ and $n \in \mathbb{N}$)
        \item \textbf{Reverse Triangle Inequality:} $\big| |x| - |y| \big| \le |x-y|$
        \item \textbf{Trigonometric Bounding:} $\sin(x) \le x \le \tan(x)$ (for $x \in (0, \pi/2)$)
    \end{itemize}
\end{theorem}
...
\end{theorem}
\subsection{Exam Strategy: Sequence \& Series Convergence}
\begin{itemize}
    \item \textbf{Type $\frac{\ln^\alpha n}{n^\beta}$:} Converges for $\beta > 0$ (any $\alpha$), or $\beta=0, \alpha < 0$.
    \item \textbf{Type $\sum \frac{1}{n \ln^\alpha n}$ (Bertrand):} Converges $\iff \alpha > 1$.
    \item \textbf{Finding $\limsup / \liminf$:} List accumulation points. If $a_n$ is defined piecewise or with $(-1)^n$, look at sub-sequences.
    \item \textbf{Radius of Convergence $R$:} Use $R = \lim |\frac{c_n}{c_{n+1}}|$ or $R = \frac{1}{\lim \sqrt[n]{|c_n|}} \in [0, \infty]$. 
        \item \textbf{Boundary Check:} Always check $|x-x_0|=R$ separately. Use Leibniz or Comparison.
    \item \textbf{Telescoping Series:} Write $a_n = b_n - b_{n+1}$. Partial sum $S_N = b_1 - b_{N+1}$.
\end{itemize}
