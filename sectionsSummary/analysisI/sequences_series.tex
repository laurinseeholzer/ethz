...
\begin{definition}
A \textbf{sequence} (Folge) $(a_n)_{n \in \mathbb{N}_0}$ is a list of numbers, mathematically a mapping 
$$f: \mathbb{N}_0 \to X$$ 
 It can be given explicitly ($a_n = f(n)$) or recursively ($a_n = g(a_{n-1}, a_{n-2}, \dots)$).
\end{definition}
...
\begin{definition}
A sequence is \textbf{monotonically increasing (decreasing)} if 
$$a_n \le (\geq) a_m \quad \forall m \gt n$$
and \textbf{upper (lower) bounded} if there exists $M$ such that 
$$|a_n| \le (\geq) M \quad \forall n \in \mathbb{N}_0$$
\end{definition}
...
\begin{definition}
A sequence is a \textbf{Cauchy sequence} if 
...
\begin{definition}
A \textbf{series} (Reihe) is an infinite sum $\sum_{i=0}^\infty a_i$. It converges iff its sequence of partial sums converges.
\end{definition}
...
\begin{remark}
\textbf{Geometric series}: $\sum_{n=0}^\infty x^n = \frac{1}{1-x}$ iff $|x| \lt 1$.
\textbf{Harmonic series}: $\sum_{n=1}^\infty \frac{1}{n^s}$ converges for $s \gt 1$.
\end{remark}
...
\begin{definition}
A series is \textbf{absolutely convergent} if $\sum |a_n|$ converges.
\\\\
...
\begin{theorem}
\textbf{Root Criterion} (Wurzelkriterium): 
$$\rho = \limsup \sqrt[n]{|a_n|}$$
If $\rho \lt 1$, abs conv; if $\rho \gt 1$, div.
\end{theorem}

\begin{theorem}
\textbf{Ratio Criterion} (Quotientenkriterium): 
