...
\textbf{Open:} Every point is an interior point (ball $B_\epsilon(x) \subset A$). Check: Is the boundary excluded? \\
\textbf{Closed:} Complement is open OR contains all limit points. Check: Is the boundary included? \\
\textbf{Bounded:} $A \subset B_R(0)$ for some $R$. \\
\textbf{Compact:} In $\mathbb{R}^n$, compact $\iff$ closed and bounded. \\
\textbf{Supremum/Infimum Tips:} For $A = \{a_n : n \in \mathbb{N}\}$, check if $a_n$ is monotonic. If $a_n \nearrow L$, then $\sup A = L$, $\inf A = a_1$. If $a_n$ oscillates, check even/odd indices separately.

\subsection{Exam Strategy: Complex Equations}
\textbf{Circles:} $|z - z_0| = R$ is a circle of radius $R$ around $z_0$. \\
\textbf{Lines:} $|z - a| = |z - b|$ is the perpendicular bisector of $a$ and $b$. \\
\textbf{Polynomials:} If $P(z)=0$ and coefficients are real, then $z \in \mathbb{C}$ is a root $\implies \bar{z}$ is also a root. \\
\textbf{Roots of Unity Sum:} $\sum_{k=0}^{n-1} z_k = 0$ for $n > 1$. \\
\textbf{Number of Solutions:} By the Fundamental Theorem of Algebra, a polynomial of degree $n \ge 1$ has exactly $n$ complex roots. \\
\textbf{Vieta's Formulas:} For polynomial $P(z) = a_n z^n + \dots + a_0$ with roots $z_1, \dots, z_n$: \textbf{Sum:} $\sum z_i = -\frac{a_{n-1}}{a_n}$. \textbf{Product:} $\prod z_i = (-1)^n \frac{a_0}{a_n}$.
