...
\subsubsection{Modelling and Definitions}
...
\subsubsection{Linear ODEs of n-th Order}
...
\subsubsection{Inhomogeneous Linear ODEs}
...
\subsubsection{ODE Solver Workflow}
\textbf{1. Can I separate?} $y' = f(x)g(y)$. \\
\textbf{2. Is it linear 1st order?} $y' + p(x)y = q(x) \to$ Integrating factor $e^{\int p}$. \\
\textbf{3. Is it 2nd order linear with constant coeffs?} $ay'' + by' + cy = s(x)$. Solve char. eq. $a\lambda^2 + b\lambda + c = 0$. Particular solution: Match $s(x)$ with Ansatz table (check for resonance!).
