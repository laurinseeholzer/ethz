...
\subsubsection{Limits in the context of functions}
...
\subsubsection{Continuity of functions}
...
\begin{definition}
    A function $f$ is \textbf{Lipschitz continuous} if there exists $L \ge 0$ such that $|f(x) - f(y)| \le L|x - y|$ for all $x, y \in D$. \\
    \textbf{Properties}: A bounded derivative $|f'(x)| \le L$ implies Lipschitz continuity. Lipschitz continuity strictly implies uniform continuity.
\end{definition}
...
\begin{theorem}
    \textbf{Intermediate Value Theorem (IVT)}: Let $f: [a, b] \to \mathbb{R}$ be a continuous function. Let $c$ be a value between $f(a)$ and $f(b)$. Then there exists a $x_0 \in [a, b]$ such that $f(x_0) = c$.
\end{theorem}
...
\textbf{Defs:} $\cosh x = \frac{e^x + e^{-x}}{2}$, $\sinh x = \frac{e^x - e^{-x}}{2}$, $\tanh x = \frac{e^x - e^{-x}}{e^x + e^{-x}}$. \\
\textbf{Identities:} $\cosh^2 x - \sinh^2 x = 1$, $\sinh(2x) = 2 \sinh x \cosh x$, $\cosh(2x) = \cosh^2 x + \sinh^2 x$. \\
\textbf{Derivatives:} $(\sinh x)' = \cosh x$, $(\cosh x)' = \sinh x$, $(\tanh x)' = \frac{1}{\cosh^2 x}$. \\
\textbf{Inverses:} $\text{arsinh}(x) = \ln(x + \sqrt{x^2+1})$, $\text{arcosh}(x) = \ln(x + \sqrt{x^2-1})$.

\subsection{Most Common Proofs}

\subsubsection{Proof Outline: Intermediate Value Theorem (IVT)}
\textbf{Goal:} $\exists x_0 \in [a,b]$ with $f(x_0) = c$. \\
\textbf{Step 1:} Assume $f(a) < c < f(b)$. Define $S = \{x \in [a,b] : f(x) \le c\}$. \\
\textbf{Step 2:} $S$ is non-empty ($a \in S$) and bounded above (by $b$). By completeness of $\mathbb{R}$, $x_0 = \sup(S)$ exists. \\
\textbf{Step 3:} Show $f(x_0) \le c$ using continuity and a sequence in $S$ converging to $x_0$. \\
\textbf{Step 4:} Show $f(x_0) \ge c$ using continuity and a sequence outside $S$ converging to $x_0$. Conclude $f(x_0) = c$. \\
\textbf{Alternative:} Bisection method. Halve $[a,b]$, pick the half where $f$ changes sign across $c$. Nested intervals shrink to $x_0$.

\subsubsection{Proof Outline: Continuity}
\textbf{Goal:} Show $f$ is continuous at $x_0$. \\
\textbf{$\epsilon-\delta$ Method:} Let $\epsilon > 0$. We need $|f(x) - f(x_0)| < \epsilon$. Manipulate $|f(x) - f(x_0)|$ to factor out $|x - x_0|$. Bound the remaining factor by $M$, assuming $|x - x_0| < 1$. Choose $\delta = \min(1, \frac{\epsilon}{M})$. Then $|x - x_0| < \delta \implies |f(x) - f(x_0)| < \epsilon$. \\
\textbf{Sequence Method:} Let $x_n \to x_0$. Show $\lim f(x_n) = f(x_0)$ using algebraic limit theorems.

\subsubsection{Proof Outline: Extreme Value Theorem}
\textbf{Goal:} Continuous $f: [a,b] \to \mathbb{R}$ attains its max and min. \\
\textbf{Boundedness:} Assume $f$ is not bounded above. Then $\exists x_n \in [a,b]$ with $f(x_n) > n$. By Bolzano-Weierstrass, $(x_n)$ has a convergent subsequence $x_{n_k} \to c \in [a,b]$. By continuity, $f(x_{n_k}) \to f(c)$, contradicting $f(x_{n_k}) > n_k \to \infty$. Thus $f$ is bounded above. Let $M = \sup f([a,b])$. \\
\textbf{Attainment:} By definition of supremum, $\exists y_n \in [a,b]$ s.t. $f(y_n) \to M$. By Bolzano-Weierstrass, $y_n$ has a convergent subsequence $y_{n_k} \to d \in [a,b]$. By continuity, $f(y_{n_k}) \to f(d) = M$. Thus max is attained at $d$.

\subsubsection{Proof Outline: Uniform Continuity (Heine-Cantor)}
\textbf{Goal:} Continuous $f$ on compact $K = [a,b]$ is uniformly continuous. \\
\textbf{Step 1:} Assume $f$ is not uniformly continuous on $K$. \\
\textbf{Step 2:} $\exists \epsilon > 0$ s.t. for all $\delta = 1/n$, there exist $x_n, y_n \in K$ with $|x_n - y_n| < 1/n$ but $|f(x_n) - f(y_n)| \ge \epsilon$. \\
\textbf{Step 3:} By Bolzano-Weierstrass, $(x_n)$ has convergent subsequence $x_{n_k} \to c \in K$. \\
\textbf{Step 4:} Since $|x_{n_k} - y_{n_k}| < 1/n_k \to 0$, $y_{n_k} \to c$ as well. \\
\textbf{Step 5:} By continuity of $f$, $f(x_{n_k}) \to f(c)$ and $f(y_{n_k}) \to f(c)$. Thus $|f(x_{n_k}) - f(y_{n_k})| \to 0$, contradicting $|f(x_{n_k}) - f(y_{n_k})| \ge \epsilon$.

\subsection{Exam Strategies}

\textbf{Prove and Disprove Uniform Continuity:} \\
\textbf{To PROVE:} Use the Heine-Cantor Theorem (a continuous function on a compact interval is always uniformly continuous) or show the function is Lipschitz continuous. \\
\textbf{To DISPROVE:} Find two sequences $x_n$ and $y_n$ such that $|x_n - y_n| \to 0$ but $|f(x_n) - f(y_n)| \ge \epsilon_0 > 0$.
