...
\begin{definition}
Let $A$ and $B$ be sets. $A$ is a subset of $B$ denoted by $A \subseteq B$ iff. $\forall x (x \in A \implies x \in B)$.
\\ \\
...
\end{itemize}
An intervall is bounded if it is bounded from above and below hence $a$ and $b$ are finite.
\end{definition}
...
\end{itemize}
The complex conjugate of $z = x + iy$ is $\bar{z} = x - iy$. Division is calculated by expanding with the conjugate of the denominator: $\frac{z}{w} = \frac{z \cdot \bar{w}}{w \cdot \bar{w}} = \frac{z \cdot \bar{w}}{|w|^2}$.
\end{definition}
