...
\begin{definition}
    $T_1$ is the execution time on a sequential machine and can be visualized as the sum of all task durations in our DAG.
\end{definition}

\begin{definition}
    $T_{\infty}$ is the execution time assuming unlimited hardware. This is called the \textbf{span} and can be visualized as the sum of the execution times of the longest dependent path in our DAG.
\end{definition}
...
\begin{remark}
    Note that arrays and balanced trees are usually parallelizable while linked lists are not even if the per element operation is parallelizable.
\end{remark}
