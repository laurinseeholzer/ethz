...
\begin{algorithm}
#Depth-First Search (DFS) & Low Values
#For efficient computation of cut vertices and edges in time $O(|V|+|E|)$.

low[v] := #the smallest DFS number reachable from $v$ by a directed path consisting of tree edges and at most one back edge.

#It holds that:
low[v] = min{ dfs[v], min_{(v,w)  back edge} dfs[w], min_{(v,w)  tree edge} low[w] }

#A vertex v is a cut vertex iff:

v != root and there exists a child u with low[u] >= df s[v].
v = root and has >= 2 children in the DFS tree.

A tree edge (v,w) is a bridge iff low[w] >= df s[v].
\end{algorithm}
