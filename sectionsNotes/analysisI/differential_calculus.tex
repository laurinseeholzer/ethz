...
\begin{remark}
    This follows directly from the product and the chain rule.
\end{remark}
...
\begin{theorem}
    ...
    $$\lim_{x \to x_0} |f(x)| = \lim_{x \to x_0} |g(x)| \in \{ 0, \infty \} \quad  \text{and} \quad \exists L = \lim_{x \to x_0} \frac{f'(x)}{g'(x)}$$
...
\begin{remark}
    This is equivalent to saying that for any $x \in (a, b)$ we have
...
\begin{proof}
    $\impliedby: \quad$ Assume $f'$ is increasing and let $x \in (a, b)$ be arbitrary. Then we have have by the mean value theorem that there exists $\psi \in (a, x) \xi \in (x, b)$ s.t.:
    $$f'(\psi) = \frac{f(x)-f(a)}{x-a} \quad \text{and} \quad f'(\xi) = \frac{f(b)-f(x)}{b-x}$$
    As $f'$ is increasing we know that $f'(\psi) \leq f'(\xi)$ and therefore
    $$\frac{f(x)-f(a)}{x-a} \leq \frac{f(b)-f(x)}{b-x}$$
    $\implies: \quad$ Assume $f$ is convex and let $h$ be small enough s.t. $x = a 
    + h \leq b-h$ then by the convexity of $f$ we have
    $$\frac{f(a+h)-f(a)}{h} \leq \frac{f(b-h)-f(a+h)}{(b-h) - (a+h)} \leq \frac{f(b)-f(b-h)}{h}$$
    and as $f$ is differentiable they exists and are comparable therfore $f'(a) \leq f'(b)$
\end{proof}
